...
Settings* g_Settings;
CVHideDrv* g_CVHideDrv;

std::string g_CVHideIniPath;
...
VOID Hide()
{
    ...
    g_CVHideDrv->Hide(HideInfo);
}

PLUG_EXPORT void CBATTACH(CBTYPE cbType, PLUG_CB_ATTACH* info) 
{
    g_CVHideDrv->SetTargetPid(info->dwProcessId);

    if (g_CVHideDrv->GetDriverHandleValue() == INVALID_HANDLE_VALUE)
        g_CVHideDrv->CreateHandleToDriver();

    if (g_CVHideDrv->GetDriverHandleValue() != INVALID_HANDLE_VALUE && Attached == FALSE)
    {
        if (g_CVHideDrv->CallDriver(IOCTL_ADD_HIDER_ENTRY) == TRUE)
            Hide();
    }
...
{
    if (g_CVHideDrv->GetDriverHandleValue() != INVALID_HANDLE_VALUE && Attached == TRUE)
        g_CVHideDrv->CallDriver(IOCTL_REMOVE_HIDER_ENTRY);
...
{
    if (g_CVHideDrv->GetDriverHandleValue() != INVALID_HANDLE_VALUE)
        g_CVHideDrv->CallDriver(IOCTL_PROCESS_STOPPED);
    Paused = TRUE;
}
...
{
    if (g_CVHideDrv->GetDriverHandleValue() != INVALID_HANDLE_VALUE)
        g_CVHideDrv->CallDriver(IOCTL_PROCESS_RESUMED);
    Paused = FALSE;
}
...
void DebugLoop(CBTYPE cbType, void* callbackInfo) 
{
    ...
    if (g_Settings->GetCurrentProfile().ClearHeapFlags == TRUE)
    {
        if (PebFix == TRUE)
        {
            g_CVHideDrv->CallDriver(IOCTL_SET_PEB_DEBUGGER_FLAG);
            PebFix = FALSE;
        }
        
        if (DebugEvent->u.LoadDll.lpBaseOfDll == NtdllModule)
        {
            g_CVHideDrv->SetTargetPid(DebugEvent->dwProcessId);
            g_CVHideDrv->CallDriver(IOCTL_CLEAR_PEB_DEBUGGER_FLAG);
            PebFix = TRUE;
        }
    }
    ...
    switch (DebugEvent->dwDebugEventCode)
    {
        ...
        case EXCEPTION_DEBUG_EVENT:
        {
            if (DebugEvent->u.Exception.ExceptionRecord.ExceptionCode == STATUS_BREAKPOINT)
            {
                ...
                if (g_CVHideDrv->GetDriverHandleValue() != INVALID_HANDLE_VALUE && Attached == FALSE && g_CVHideDrv->CallDriver(IOCTL_ADD_HIDER_ENTRY) == TRUE)
                    Hide();
        ...
        case WM_COMMAND:
        {
            switch (LOWORD(wParam))
            {
                case IDC_BTN_OK: 
                {
                    ...
                    MessageBoxW(hDlg, L"Settings applied!", L"[CVHide Options]", MB_ICONINFORMATION);
...
void MenuEntry(CBTYPE cbType, void* CallbackInfo)
{
    ...
    switch (Info->hEntry)
    {
        case MENU_OPTIONS:
        {
            if (g_CVHideDrv->GetDriverHandleValue() == INVALID_HANDLE_VALUE)
            {
                if (g_CVHideDrv->CreateHandleToDriver() == FALSE)
                {
                    MessageBoxA(hwndDlg, "Couldn't establish connection with CVHide Driver\r\n", "Error", MB_OK);
                    break;
                }
            }
    ...
    LPVOID lpReserved) 
{
    switch (fdwReason)
    {
        ...
        {
            hinst = hinstDLL;
            g_CVHideIniPath = GetModulePath(hinstDLL);
            g_CVHideIniPath.resize(g_CVHideIniPath.find_last_of(L'\\') + 1);
